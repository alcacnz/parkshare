...
async function updateScheduleDB(spot_id, day_of_week, owner) {
  await fetch(`${SUPABASE_URL}/rest/v1/weekly_schedule?spot_id=eq.${spot_id}&day_of_week=eq.${day_of_week}`, {
    method: "PATCH", headers: { ...HEADERS, "Prefer": "return=minimal" }, body: JSON.stringify({ owner }),
  });
}
...
function maxEnd(start) { const d = new Date(start + "T00:00:00"); d.setDate(d.getDate() + 19); return d.toLocaleDateString("en-CA"); }
function dayCount(from, until) { return Math.round((new Date(until + "T00:00:00") - new Date(from + "T00:00:00")) / 86400000) + 1; }
...
function computeStatus(spot) {
  ...
  const today = todayNZ();
  const inRange = spot.released_from && spot.released_until && today >= spot.released_from && today <= spot.released_until;
...
function SpotTile({ spot, selected, onSelect, isLast }) {
  ...
  return (
    <div onClick={() => onSelect(spot)} style={{ flex: 1, minWidth: 36, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 2, padding: "4px 0", borderRight: isLast ? "none" : "1px solid #444444", background: sel ? "rgba(255,255,255,0.08)" : "transparent", transition: "background 0.15s" }}>
      ...
      <div style={{ width: 36, height: 52, borderRadius: 3, background: c.bg, border: `1px solid ${c.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
        {status === "available" && (
  ...
  return (
    <div style={{ flex: 1, minWidth: 36, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, padding: "4px 0", borderRight: "1px solid #444444" }}>
      ...
      <div style={{ width: 36, height: 52, borderRadius: 3, background: "#888888", border: "1px solid #666666", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: 8, color: "#dddddd", fontWeight: 700, textAlign: "center" }}>MAIN DOOR</span>
      </div>
...
function SpotPanel({
  spot, isAdmin, editName, setEditName,
  bookingName, setBookingName,
  releaseName, setReleaseName, releaseNameError, setReleaseNameError,
  showReleaseNameInput, setShowReleaseNameInput,
  showBookingInput, setShowBookingInput,
  showConfirmRelease, setShowConfirmRelease,
  onClose, onBook, onRelease, onCancelRelease, onReleaseBooking,
  onAdminAssign, onAdminClearOwner, onAdminCancelBooking,
}) {
  if (!spot) return null;
  const status = computeStatus(spot);
    ...
    <div style={{ background: "white", borderRadius: 12, padding: 16, marginBottom: 16, boxShadow: "0 2px 16px rgba(0,0,0,0.13)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>{dedicated ? `Dedicated — ${spot.owner}` : "Open parking"}</div>
      ...
      {isAdmin ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ borderBottom: "1px solid #f0f0f0", paddingBottom: 12 }}>
            ...
            <input value={editName} onChange={e => setEditName(e.target.value)} placeholder="Staff name or leave blank"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {status === "available" && (
            showBookingInput ? (
              <>
                ...
                <input value={bookingName} onChange={e => setBookingName(e.target.value)} placeholder="Your name" autoFocus
          ...
          {status === "reserved" && (
            showReleaseNameInput ? (
              <>
                <p style={{ margin: 0, fontSize: 12, color: "#555", fontWeight: 600 }}>Enter your name to confirm release</p>
                <input value={releaseName} onChange={e => { setReleaseName(e.target.value); setReleaseNameError(false); }} placeholder="Your name (case-sensitive)" autoFocus
                  style={{ width: "100%", border: `1px solid ${releaseNameError ? "#c00" : "#ddd"}`, borderRadius: 6, padding: "8px 10px", fontSize: 13, boxSizing: "border-box" }} />
                {releaseNameError && <p style={{ color: "#c00", fontSize: 12, margin: 0 }}>Name does not match. Release blocked.</p>}
                <button onClick={onRelease} style={btn("#085041", "white", { marginBottom: 0 })}>Confirm release</button>
                <button onClick={() => { setShowReleaseNameInput(false); setReleaseName(""); setReleaseNameError(false); }} style={btn("#f3f4f6", "#555", { marginBottom: 0 })}>Cancel</button>
              </>
            ) : (
              <>
                <div style={{ background: "#FFF8E1", borderRadius: 8, padding: "10px 12px", marginBottom: 4 }}>
                  <p style={{ color: "#92620A", fontWeight: 600, fontSize: 13, margin: "0 0 2px" }}>This is {spot.owner ? spot.owner + "'s" : "a"} spot</p>
                  <p style={{ color: "#92620A", fontSize: 11, margin: 0 }}>Enter your name to proceed</p>
                </div>
                <button onClick={() => setShowReleaseNameInput(true)} style={btn("#085041", "white", { marginBottom: 0 })}>Release my spot</button>
              </>
            )
          )}
          {status === "booked" && (
            showConfirmRelease ? (
              <>
                ...
                <input value={releaseName} onChange={e => { setReleaseName(e.target.value); setReleaseNameError(false); }} placeholder="Your name (case-sensitive)" autoFocus
                  style={{ width: "100%", border: `1px solid ${releaseNameError ? "#c00" : "#ddd"}`, borderRadius: 6, padding: "8px 10px", fontSize: 13, boxSizing: "border-box" }} />
                {releaseNameError && <p style={{ color: "#c00", fontSize: 12, margin: 0 }}>Name does not match. Release blocked.</p>}
                <button onClick={onReleaseBooking} style={btn("#0F6E56", "white", { marginBottom: 0 })}>Confirm release</button>
                <button onClick={() => { setShowConfirmRelease(false); setReleaseName(""); setReleaseNameError(false); }} style={btn("#f3f4f6", "#555", { marginBottom: 0 })}>Cancel</button>
              </>
            ) : (
              <>
...
function ScheduleManager({ spots, onClose, showToast }) {
  ...
  const handleChange = (spotId, value) => {
    setSchedule(prev => ({ ...prev, [activeDay + 1]: { ...(prev[activeDay + 1] || {}), [spotId]: value } }));
  };
  ...
  const allSpots = spots.filter(s => s.section === "front" || s.section === "back")
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: 16 }}>
      <div style={{ background: "white", borderRadius: 14, padding: 20, width: "100%", maxWidth: 480, maxHeight: "85vh", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
          {DAYS.map((day, i) => (
            <button key={day} onClick={() => setActiveDay(i)} style={{ flex: 1, padding: "8px 0", border: "none", borderRadius: 6, cursor: "pointer", background: activeDay === i ? "#0F6E56" : "#f3f4f6", color: activeDay === i ? "white" : "#555", fontWeight: 600, fontSize: 13 }}>{day}</button>
          ))}
        </div>
          ...
          <div style={{ overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
            {allSpots.map(spot => (
              <div key={spot.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <input value={(schedule[activeDay + 1] || {})[spot.id] || ""} onChange={e => handleChange(spot.id, e.target.value)}
        ...
        <button onClick={handleSave} disabled={saving} style={{ marginTop: 14, background: "#0F6E56", color: "white", border: "none", borderRadius: 7, padding: "11px", cursor: "pointer", fontWeight: 600, fontSize: 14, opacity: saving ? 0.7 : 1 }}>
...
const spotRow = (spotsArr, selected, selectSpot) => (
...
export default function ParkShare() {
  ...
  const resetPanel = () => {
    ...
    setShowReleaseNameInput(false); setBookingName("");
    setReleaseName(""); setReleaseNameError(false);
  };
  ...
  const handleBook = async () => {
    ...
    if (current && current.booked_by) {
      showToast("This spot was just booked by someone else!");
      resetPanel(); setSpots(latest); return;
    }
  ...
  const handleRelease = async () => {
    if (!selected) return;
    if (releaseName !== selected.owner) { setReleaseNameError(true); return; }
    const today = todayNZ();
  ...
  const handleReleaseBooking = async () => {
    if (!selected) return;
    if (releaseName !== selected.booked_by) { setReleaseNameError(true); return; }
  ...
  return (
    <div style={{ background: "#0F6E56", minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "20px 16px 80px" }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              {isAdmin ? (
                ...
                <button onClick={() => setShowAdminBox(true)} style={{ background: "transparent", border: "1px solid #5DCAA5", color: "#5DCAA5", borderRadius: 6, padding: "6px 10px", cursor: "pointer", fontSize: 12 }}>Admin</button>
              )}
            </div>
          </div>
        </div>
              ...
              <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 4, width: "50%", flexShrink: 0 }}>
                    ...
                    <div>{spotRow(backSpots, selected, selectSpot)}</div>
                  </div>
                ...
                {/* Driveway + Temp + Small driveway + Great South Road */}
        ...
        {/* Schedule Manager */}
        {showSchedule && <ScheduleManager spots={spots} onClose={() => setShowSchedule(false)} showToast={showToast} />}
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}>
            <div style={{ background: "white", borderRadius: 14, padding: 20, maxWidth: 320, width: "100%" }}>
              ...
              <ul style={{ fontSize: 13, color: "#444", paddingLeft: 18, margin: "0 0 14px", lineHeight: 1.9 }}>
                ...
                <li>Release your spot — name verification required</li>
